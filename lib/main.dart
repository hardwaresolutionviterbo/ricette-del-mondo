...
class _AppShellState extends State<AppShell>{
 ...
 Widget home(){
  ...
  final italian=all.where((r)=>r.country.toLowerCase()=='italia').length;
      ...
      SizedBox(
        ...
        child:ListView(
          ...
          children:[
            ...
            specialCompact('assets/banners/banner_braceria.png','Braceria','Il gusto autentico della griglia',()=>openSpecialPage('Speciale Braceria','Ricette per griglia, brace e cotture lente.',[...specialBraceriaRecipes,...catalog.where((r)=>r.tags.any((t)=>['carne','griglia','brace','bbq','pollo'].contains(t.toLowerCase()))||r.title.toLowerCase().contains('asado')||r.title.toLowerCase().contains('pulled'))],Icons.outdoor_grill)),
            specialCompact('assets/banners/banner_gourmet.png','Gourmet','L’alta cucina a casa tua',()=>openSpecialPage('Speciale Gourmet Stellato','Una raccolta editoriale di alta cucina.',catalog.where((r)=>r.tags.any((t)=>t.toLowerCase()=='gourmet')).toList(),Icons.auto_awesome)),
          ],
        ),
      ),
 ...
 Widget searchPage()=>ListView(padding:const EdgeInsets.fromLTRB(16,10,16,28),children:[
   ...
   Wrap(spacing:8,runSpacing:8,children:['Pasta','Pollo','Pizza','Dolci','Vegetariano','Senza glutine','Salmone','Veloci'].map((x)=>modernChip(x,selected:search.text.toLowerCase()==x.toLowerCase(),onTap:(){search.text=x;setState((){});})).toList()),
