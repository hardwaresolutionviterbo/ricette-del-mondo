...
class _SplashPageState extends State<SplashPage> with SingleTickerProviderStateMixin{
 ...
 @override void initState(){super.initState();controller=AnimationController(vsync:this,duration:const Duration(milliseconds:4200))..forward();Future.delayed(const Duration(milliseconds:4400),(){if(mounted)Navigator.of(context).pushReplacement(MaterialPageRoute(builder:(_)=>const AppShell()));});}
 @override void dispose(){controller.dispose();super.dispose();}
 @override Widget build(BuildContext c)=>Scaffold(backgroundColor:Colors.black,body:SafeArea(child:LayoutBuilder(builder:(c,box)=>Stack(children:[
   Positioned.fill(child:Image.asset('assets/splash_v6.png',fit:BoxFit.fill,filterQuality:FilterQuality.medium)),
   Positioned(left:0,right:0,bottom:0,height:96,child:IgnorePointer(child:DecoratedBox(decoration:BoxDecoration(gradient:LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[Colors.transparent,Colors.black.withValues(alpha:.18)]))))),
   Positioned(left:28,right:28,bottom:22,child:AnimatedBuilder(animation:controller,builder:(_,__) {final p=controller.value;return Align(alignment:Alignment.center,child:Opacity(opacity:.16+.34*p,child:const SizedBox(height:2,width:72,child:DecoratedBox(decoration:BoxDecoration(color:Colors.white)))));}) )
 ]))));}
