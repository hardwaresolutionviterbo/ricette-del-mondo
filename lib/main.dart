...
class _SplashPageState extends State<SplashPage>{double progress=0;@override void initState(){super.initState();Future.delayed(const Duration(milliseconds:1900),(){if(mounted)Navigator.of(context).pushReplacement(MaterialPageRoute(builder:(_)=>const AppShell()));});}
 @override Widget build(BuildContext c)=>Scaffold(backgroundColor:cream,body:SafeArea(child:Stack(children:[Positioned.fill(child:Container(decoration:const BoxDecoration(gradient:LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[cream,Color(0xFFF6E9D2)])))),Center(child:Padding(padding:const EdgeInsets.symmetric(horizontal:28),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[TweenAnimationBuilder<double>(tween:Tween(begin:.82,end:1),duration:const Duration(milliseconds:900),curve:Curves.easeOutBack,builder:(_,v,__)=>Transform.scale(scale:v,child:Image.asset('assets/logo_rdm.png',height:235,fit:BoxFit.contain))),const SizedBox(height:10),const Text('Più di una ricetta, è un viaggio!',textAlign:TextAlign.center,style:TextStyle(fontSize:21,fontWeight:FontWeight.w800,color:green)),const SizedBox(height:8),const Text('IL PIÙ GRANDE RICETTARIO DEL MONDO, SEMPRE CON TE',textAlign:TextAlign.center,style:TextStyle(fontSize:14,fontWeight:FontWeight.w700,letterSpacing:1.1,color:ink)),const SizedBox(height:26),TweenAnimationBuilder<double>(tween:Tween(begin:0,end:1),duration:const Duration(milliseconds:1800),curve:Curves.easeInOut, builder:(_,v,__){return Column(children:[ClipRRect(borderRadius:BorderRadius.circular(20),child:LinearProgressIndicator(value:v,minHeight:10,backgroundColor:Colors.white,valueColor:const AlwaysStoppedAnimation<Color>(green))),const SizedBox(height:7),Text('${(v*100).round()}%',style:const TextStyle(fontWeight:FontWeight.w800,color:green))]);}),const SizedBox(height:24),const Text('Stiamo preparando un mondo di ricette per te…',textAlign:TextAlign.center,style:TextStyle(color:ink,fontSize:13))])))])));}
