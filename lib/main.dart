...
Future<void> shareRecipe(BuildContext context, Recipe r) async {
  final text='${flag(r.country)} ${r.title}\n\nScopri la ricetta su Ricette del Mondo.\nOgni ricetta è un viaggio.';
  await SharePlus.instance.share(ShareParams(title:r.title,text:text));
}
...
class _SplashPageState extends State<SplashPage>{int recipeCount=0;@override void initState(){super.initState();RecipeRepository().loadRecipes().then((r){if(mounted)setState(()=>recipeCount=r.length);});Future.delayed(const Duration(milliseconds:2500),(){if(mounted)Navigator.of(context).pushReplacement(MaterialPageRoute(builder:(_)=>const AppShell()));});}
 @override Widget build(BuildContext c)=>Scaffold(backgroundColor:green,body:SafeArea(child:Container(decoration:const BoxDecoration(gradient:LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[Color(0xFF063D29),green,Color(0xFF0B6A45)])),child:Stack(children:[Positioned(top:28,left:22,child:Icon(Icons.public,size:52,color:Colors.white24)),Positioned(top:80,right:20,child:Icon(Icons.flight_takeoff_rounded,size:38,color:Colors.white24)),Positioned(bottom:55,left:18,child:Icon(Icons.restaurant_rounded,size:42,color:Colors.white24)),Center(child:Padding(padding:const EdgeInsets.symmetric(horizontal:28),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[Container(padding:const EdgeInsets.all(18),decoration:BoxDecoration(color:Colors.white.withOpacity(.97),shape:BoxShape.circle,boxShadow:[BoxShadow(color:Colors.black.withOpacity(.18),blurRadius:24,offset:const Offset(0,10))]),child:Image.asset('assets/logo_rdm.png',height:205,fit:BoxFit.contain)),const SizedBox(height:20),const Text('Ricette del Mondo',textAlign:TextAlign.center,style:TextStyle(fontSize:30,fontWeight:FontWeight.w900,color:Colors.white)),const SizedBox(height:6),const Text('SAPORI SENZA CONFINI',style:TextStyle(fontSize:13,fontWeight:FontWeight.w800,letterSpacing:2.2,color:orange)),const SizedBox(height:14),const Text('Più di una ricetta, è un viaggio.',textAlign:TextAlign.center,style:TextStyle(fontSize:17,fontWeight:FontWeight.w700,color:Colors.white)),const SizedBox(height:22),Container(width:double.infinity,padding:const EdgeInsets.symmetric(horizontal:18,vertical:12),decoration:BoxDecoration(color:Colors.white.withOpacity(.12),borderRadius:BorderRadius.circular(18),border:Border.all(color:Colors.white24)),child:Row(mainAxisAlignment:MainAxisAlignment.center,children:[const Icon(Icons.menu_book_rounded,color:orange,size:22),const SizedBox(width:8),Text(recipeCount==0?'Caricamento ricette…':'$recipeCount ricette caricate',style:const TextStyle(color:Colors.white,fontSize:17,fontWeight:FontWeight.w900))])),const SizedBox(height:22),TweenAnimationBuilder<double>(tween:Tween(begin:0,end:1),duration:const Duration(milliseconds:2300),curve:Curves.easeInOut,builder:(_,v,__){return Column(children:[ClipRRect(borderRadius:BorderRadius.circular(20),child:LinearProgressIndicator(value:v,minHeight:10,backgroundColor:Colors.white24,valueColor:const AlwaysStoppedAnimation<Color>(orange))),const SizedBox(height:7),Text('${(v*100).round()}%',style:const TextStyle(fontWeight:FontWeight.w800,color:Colors.white))]);}),const SizedBox(height:12),const Text('Stiamo preparando il tuo viaggio culinario…',textAlign:TextAlign.center,style:TextStyle(color:Colors.white70,fontSize:12))])))]))));}
...
class _AppShellState extends State<AppShell>{
 ...
 Widget home(){final total=all.length;final italian=all.where((r)=>r.country.toLowerCase()=='italia').length;final world=total-italian;return ListView(padding:const EdgeInsets.fromLTRB(16,10,16,30),children:[homeTopBar(),homeHero(total,italian,world),const SizedBox(height:14),searchBox(),const SizedBox(height:14),fridgeBanner(),section('Speciali di Ricette del Mondo'),specialBanner('assets/banners/banner_impasti.png',onTap:()=>openSpecialPage('Speciale Impasti','Impasti per pizza e focaccia da fare a casa.',specialDoughRecipes,Icons.local_pizza)),specialBanner('assets/banners/banner_hamburger.png',onTap:()=>openSpecialPage('Speciale Hamburger','Una raccolta dedicata agli hamburger.',[...specialHamburgerRecipes,...catalog.where((r)=>r.title.toLowerCase().contains('hamburger')||r.tags.any((t)=>t.toLowerCase().contains('burger')))],Icons.lunch_dining)),specialBanner('assets/banners/banner_braceria.png',onTap:()=>openSpecialPage('Speciale Braceria','Ricette per griglia, brace e cotture lente.',[...specialBraceriaRecipes,...catalog.where((r)=>r.tags.any((t)=>['carne','griglia','brace','bbq','pollo'].contains(t.toLowerCase()))||r.title.toLowerCase().contains('asado')||r.title.toLowerCase().contains('pulled'))],Icons.outdoor_grill)),specialBanner('assets/banners/banner_gourmet.png',onTap:()=>openSpecialPage('Speciale Gourmet Stellato','Una raccolta editoriale di alta cucina.',catalog.where((r)=>r.tags.any((t)=>t.toLowerCase()=='gourmet')).toList(),Icons.auto_awesome)),section('Le 10 ricette gratuite',action:'Vedi tutte',onAction:()=>setState(()=>tab=1)),SizedBox(height:264,child:ListView(scrollDirection:Axis.horizontal,padding:const EdgeInsets.only(bottom:4),children:all.take(10).map((r)=>miniCard(r)).toList())),ratingHomeSection('🏆 Ricette più votate',false),ratingHomeSection('😋 Ricette più buone',true),section('Esplora il mondo'),continentGrid(),section('Seguici sui social'),socialSection(),section('Scopri Premium'),premiumBanner(),section('Idee per te'),...all.skip(10).take(5).map(recipeCard)]);}
 Widget homeTopBar()=>Padding(padding:const EdgeInsets.only(bottom:10),child:Row(children:[Container(padding:const EdgeInsets.symmetric(horizontal:12,vertical:7),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(18),boxShadow:[BoxShadow(color:Colors.black12,blurRadius:8,offset:Offset(0,3))]),child:Image.asset('assets/logo_rdm.png',height:58,fit:BoxFit.contain)),const Spacer(),IconButton(style:IconButton.styleFrom(backgroundColor:Colors.white),onPressed:()=>setState(()=>tab=3),icon:const Icon(Icons.favorite_border,color:green)),const SizedBox(width:5),IconButton(style:IconButton.styleFrom(backgroundColor:Colors.white),onPressed:()=>setState(()=>tab=4),icon:const Icon(Icons.person_outline,color:green))]));
 Widget homeHero(int total,int italian,int world){
  return Card(
    ...
    child: Container(
      ...
      decoration: const BoxDecoration(gradient: LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[green,Color(0xFF0C7A4B)])),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children:[
            Container(width:46,height:46,decoration:const BoxDecoration(color:Colors.white12,shape:BoxShape.circle),child:const Icon(Icons.public,color:orange,size:27)),
            const SizedBox(width:12),
            const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              Text('SAPORI SENZA CONFINI',style:TextStyle(color:orange,fontSize:11,fontWeight:FontWeight.w900,letterSpacing:1.4)),
              SizedBox(height:3),
              Text('Il tuo viaggio in cucina',style:TextStyle(color:Colors.white,fontSize:22,fontWeight:FontWeight.w900)),
            ])),
          ]),
          const SizedBox(height:14),
          const Text('Ogni ricetta è un viaggio.',style:TextStyle(color:Colors.white,fontSize:15,fontWeight:FontWeight.w700)),
          const SizedBox(height:14),
          Container(
          ...
          Row(children:[
            ...
            Expanded(child:quickHomeAction(Icons.favorite_border,'I miei preferiti',()=>setState(()=>tab=3))),
          ]),
        ],
      ),
    ),
 ...
 Widget countStat(String value,String label)=>Column(children:[Text(value,style:const TextStyle(fontSize:22,fontWeight:FontWeight.w900,color:green)),Text(label,style:const TextStyle(fontSize:9,fontWeight:FontWeight.w800,color:Colors.white70,letterSpacing:.5))]);
 ...
 Widget specialBanner(String assetPath,{required VoidCallback onTap})=>Card(clipBehavior:Clip.antiAlias,elevation:3,margin:const EdgeInsets.only(bottom:12),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(24)),child:InkWell(onTap:onTap,borderRadius:BorderRadius.circular(24),child:AspectRatio(aspectRatio:4.72,child:Image.asset(assetPath,fit:BoxFit.cover,filterQuality:FilterQuality.high,errorBuilder:(_,__,___)=>Container(color:pale,alignment:Alignment.center,child:const Icon(Icons.image_not_supported_outlined,color:green,size:34))))));
 ...
 Widget ratingRow(Recipe r,bool taste)=>Card(margin:const EdgeInsets.only(bottom:8),child:ListTile(leading:SizedBox(width:58,height:58,child:recipeVisual(r,height:58,radius:BorderRadius.circular(12))),title:Text(r.title,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontWeight:FontWeight.w900,color:ink)),subtitle:Row(children:[stars(avgFor(r,taste:taste)),const SizedBox(width:5),Text('${avgFor(r,taste:taste).toStringAsFixed(1)} • ${countFor(r,taste:taste)} voti',style:const TextStyle(fontSize:11))]),trailing:Row(mainAxisSize:MainAxisSize.min,children:[IconButton(tooltip:'Condividi ricetta',onPressed:()=>shareRecipe(c,r),icon:const Icon(Icons.share_outlined,color:green)),const Icon(Icons.chevron_right)]),onTap:()=>openRecipe(r)));
 ...
 Widget miniCard(Recipe r)=>SizedBox(width:172,height:254,child:Container(margin:const EdgeInsets.only(right:12),child:Card(clipBehavior:Clip.antiAlias,elevation:2,child:InkWell(onTap:()=>openRecipe(r),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[recipeVisual(r,height:112,radius:BorderRadius.zero),Padding(padding:const EdgeInsets.fromLTRB(10,7,6,6),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Expanded(child:Text(r.title,maxLines:2,overflow:TextOverflow.ellipsis,style:const TextStyle(fontWeight:FontWeight.w900,color:ink))),IconButton(padding:EdgeInsets.zero,constraints:const BoxConstraints(minWidth:34,minHeight:34),tooltip:'Condividi ricetta',onPressed:()=>shareRecipe(context,r),icon:const Icon(Icons.share_outlined,size:20,color:green))]),Text('${flag(r.country)} ${r.country}',style:const TextStyle(fontSize:12)),const SizedBox(height:3),Text('${r.timeMin} min • ${cost(r,r.servings)}',maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:11))]))])))));
              ...
              child: Column(
                ...
                children: [
                  Row(crossAxisAlignment:CrossAxisAlignment.start,children:[
                    Expanded(child:Text(r.title,maxLines:2,overflow:TextOverflow.ellipsis,style:const TextStyle(fontWeight:FontWeight.w900,fontSize:16,color:ink))),
                  ...
                  Text('${r.prepMin} min prep • ${r.cookMin} min cottura • ${r.servings} porzioni',style:const TextStyle(fontSize:11)),
                  Text(cost(r,r.servings),style:const TextStyle(fontSize:11,color:green,fontWeight:FontWeight.w800)),
                  Align(alignment:Alignment.centerRight,child:IconButton(
                    ...
                    onPressed:()=>shareRecipe(context,r),
                    icon:const Icon(Icons.share_outlined,color:green),
                  )),
                ],
              ),
            ),
  ...
  Widget row(String title, double value, int count, ValueChanged<int> onTap) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 5),
        Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Wrap(spacing:2,children:List.generate(5,(i)=>IconButton(padding:EdgeInsets.zero,constraints:const BoxConstraints(minWidth:38,minHeight:38),onPressed:()=>onTap(i+1),icon:Icon(i<value.round()?Icons.star:Icons.star_border,color:orange,size:27)))),const SizedBox(height:2),Text(count==0?'Ancora nessun voto':'${value.toStringAsFixed(1)} / 5 • $count voti',maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:12,color:Colors.black54))]),
      ],
    );
  }
            ...
            ...recipes.map(
              (r) => Card(
                child: ListTile(
                  ...
                  trailing: Row(mainAxisSize:MainAxisSize.min,children:[IconButton(tooltip:'Condividi ricetta',onPressed:()=>shareRecipe(c,r),icon:const Icon(Icons.share_outlined,color:green)),const Icon(Icons.chevron_right)]),
