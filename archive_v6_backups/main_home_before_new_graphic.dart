  ...
  return Card(
    ...
    child:InkWell(
      ...
      child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        Stack(children:[
          SizedBox(width:double.infinity,height:190,child:recipeVisual(recipe,height:190,radius:BorderRadius.zero)),
          Positioned(left:12,top:12,child:Container(padding:const EdgeInsets.symmetric(horizontal:11,vertical:7),decoration:BoxDecoration(color:green,borderRadius:BorderRadius.circular(18),boxShadow:[BoxShadow(color:Colors.black26,blurRadius:8)]),child:const Text('⭐ RICETTA DEL GIORNO',style:TextStyle(color:Colors.white,fontSize:11,fontWeight:FontWeight.w900,letterSpacing:.4)))),
        ]),
        Padding(padding:const EdgeInsets.fromLTRB(15,12,15,15),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          Row(children:[Expanded(child:Text(recipe.title,maxLines:2,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:21,fontWeight:FontWeight.w900,color:ink))),const SizedBox(width:8),const Icon(Icons.arrow_forward_ios_rounded,size:17,color:green)]),
          const SizedBox(height:5),
          Text('${flag(recipe.country)} ${recipe.country} • ${recipe.cuisine}',maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:12,fontWeight:FontWeight.w700,color:green)),
          const SizedBox(height:7),
          Row(children:[const Icon(Icons.calendar_today_outlined,size:14,color:Colors.black54),const SizedBox(width:5),Text(dateLabel,style:const TextStyle(fontSize:11,color:Colors.black54)),const SizedBox(width:12),Text('${recipe.timeMin} min',style:const TextStyle(fontSize:11,color:Colors.black54)),const SizedBox(width:12),Text(recipe.difficulty,style:const TextStyle(fontSize:11,color:Colors.black54))]),
          const SizedBox(height:10),
          const Text('Una nuova idea ogni giorno. Tocca per aprire la ricetta completa.',style:TextStyle(fontSize:12.5,fontWeight:FontWeight.w700,color:ink)),
        ])),
      ]),
    ),
  );
 }
 Widget homeHero(int total,int italian,int world)=>Container(
  decoration:BoxDecoration(
    color:cream,
    borderRadius:BorderRadius.circular(32),
    boxShadow:[BoxShadow(color:Colors.black.withOpacity(.10),blurRadius:22,offset:const Offset(0,10))],
    ...
    child:Column(children:[
      Container(
        ...
        decoration:const BoxDecoration(
          gradient:LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[Color(0xFF083F2B),Color(0xFF0B6542),Color(0xFF174A38)]),
        ),
        child:Column(children:[
          Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[
            Container(
              padding:const EdgeInsets.symmetric(horizontal:13,vertical:8),
              decoration:BoxDecoration(color:Colors.white.withOpacity(.10),borderRadius:BorderRadius.circular(22),border:Border.all(color:Colors.white.withOpacity(.10))),
              child:const Text('SAPORI SENZA CONFINI',style:TextStyle(fontSize:10,fontWeight:FontWeight.w900,letterSpacing:1.5,color:Colors.white)),
            ),
            IconButton(
              tooltip:'Profilo',
              style:IconButton.styleFrom(backgroundColor:Colors.white.withOpacity(.12),foregroundColor:Colors.white),
          ...
          Container(
            ...
            decoration:BoxDecoration(shape:BoxShape.circle,color:Colors.white.withOpacity(.06),border:Border.all(color:Colors.white.withOpacity(.10),width:1.5)),
            padding:const EdgeInsets.all(9),
            child:Image.asset('assets/logo_rdm.png',fit:BoxFit.contain),
          ),
          const SizedBox(height:10),
          const Text('Ogni ricetta è un viaggio.',textAlign:TextAlign.center,style:TextStyle(fontSize:21,fontWeight:FontWeight.w900,color:Colors.white,letterSpacing:.1)),
          const SizedBox(height:5),
          Text('Scopri sapori, tradizioni e cucine da tutto il mondo.',textAlign:TextAlign.center,style:TextStyle(fontSize:12.5,fontWeight:FontWeight.w500,color:Colors.white.withOpacity(.82))),
        ]),
      ),
      Padding(
        ...
        child:Column(children:[
          Container(
            ...
            decoration:BoxDecoration(color:const Color(0xFFF1E6D5),borderRadius:BorderRadius.circular(22),border:Border.all(color:const Color(0xFFE4D2B9))),
            child:Row(mainAxisAlignment:MainAxisAlignment.spaceEvenly,children:[
              countStat('$total','RICETTE'),
              Container(width:1,height:42,color:const Color(0xFFD7C4AA)),
              countStat('$italian','ITALIANE'),
              Container(width:1,height:42,color:const Color(0xFFD7C4AA)),
              countStat('$world','DAL MONDO'),
            ]),
          ),
          const SizedBox(height:12),
          Row(children:[
            Expanded(child:quickHomeAction(Icons.kitchen_outlined,'Cosa hai nel frigo?',fridgePage)),
            const SizedBox(width:10),
            Expanded(child:quickHomeAction(Icons.favorite_border_rounded,'I miei preferiti',()=>setState(()=>tab=3))),
          ]),
        ]),
      ),
    ]),
 ...
 Widget profilePage()=>ListView(padding:const EdgeInsets.fromLTRB(16,8,16,28),children:[
  Container(
    ...
    decoration:BoxDecoration(
      ...
      boxShadow:[BoxShadow(color:Colors.black.withOpacity(.07),blurRadius:18,offset:const Offset(0,7))],
    ),
    child:Column(children:[
      Row(children:[
      ...
      Row(crossAxisAlignment:CrossAxisAlignment.center,children:[
        Stack(clipBehavior:Clip.none,children:[
          CircleAvatar(radius:48,backgroundColor:const Color(0xFFDCEFE5),child:const Icon(Icons.person_rounded,size:56,color:green)),
          Positioned(right:-2,bottom:0,child:Container(width:31,height:31,decoration:const BoxDecoration(color:green,shape:BoxShape.circle),child:const Icon(Icons.edit_rounded,size:16,color:Colors.white))),
        ]),
  ...
  const SizedBox(height:14),
  Container(padding:const EdgeInsets.symmetric(vertical:14),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(24),boxShadow:[BoxShadow(color:Colors.black.withOpacity(.06),blurRadius:14,offset:const Offset(0,5))]),child:Row(children:[
...
class _PremiumPageState extends State<PremiumPage>{
 int selectedPlan=1;
 final plans=const [('1 mese','€2,99','Flessibile','Rinnovo mensile'),('6 mesi','€14,99','Più scelto','Risparmi rispetto al mensile'),('12 mesi','€24,99','Miglior valore','Il prezzo migliore')];
 @override Widget build(BuildContext c)=>Scaffold(backgroundColor:cream,appBar:AppBar(title:const Text('Premium',style:TextStyle(fontWeight:FontWeight.w900))),body:ListView(padding:const EdgeInsets.fromLTRB(16,8,16,28),children:[
 Container(padding:const EdgeInsets.fromLTRB(22,24,22,26),decoration:BoxDecoration(gradient:const LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[Color(0xFF063D28),Color(0xFF0C7A4B)]),borderRadius:BorderRadius.circular(30),boxShadow:[BoxShadow(color:Colors.black26,blurRadius:16,offset:Offset(0,8))]),child:Column(children:[const Icon(Icons.workspace_premium,size:58,color:orange),const SizedBox(height:8),const Text('Ricette del Mondo Premium',textAlign:TextAlign.center,style:TextStyle(color:Colors.white,fontSize:28,fontWeight:FontWeight.w900)),const SizedBox(height:7),const Text('Più ricette. Più viaggi. Più sapori.',textAlign:TextAlign.center,style:TextStyle(color:Color(0xFFFFD889),fontSize:18,fontWeight:FontWeight.w700)),const SizedBox(height:8),const Text('Un mondo di ricette esclusive, raccolte speciali e strumenti per organizzare la tua cucina.',textAlign:TextAlign.center,style:TextStyle(color:Colors.white70,fontSize:14))])),
 const SizedBox(height:18),Card(color:Colors.white,elevation:1,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(24)),child:Padding(padding:const EdgeInsets.fromLTRB(12,8,12,10),child:Column(children:[const Padding(padding:EdgeInsets.all(8),child:Row(children:[Icon(Icons.workspace_premium,color:orange),SizedBox(width:8),Expanded(child:Text('Cosa ottieni con Premium',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900,color:ink)))])),...['Accesso a tutte le ricette Premium','Ingredienti e procedimenti completi','Storie, varianti e curiosità','Raccolte speciali: Impasti, Hamburger e Braceria','Lista della spesa e funzione frigo','Esperienza senza pubblicità'].map((x)=>ListTile(dense:true,leading:const Icon(Icons.check_circle,color:green),title:Text(x,style:const TextStyle(fontWeight:FontWeight.w600))))])),
 const SizedBox(height:18),const Text('Scegli il tuo abbonamento',style:TextStyle(fontSize:22,fontWeight:FontWeight.w900,color:ink)),const SizedBox(height:8),
 ...List.generate(plans.length,(i){final p=plans[i];final sel=selectedPlan==i;return Padding(padding:const EdgeInsets.only(bottom:10),child:InkWell(borderRadius:BorderRadius.circular(20),onTap:()=>setState(()=>selectedPlan=i),child:AnimatedContainer(duration:const Duration(milliseconds:180),padding:const EdgeInsets.all(16),decoration:BoxDecoration(color:sel?const Color(0xFFFFF4DE):Colors.white,borderRadius:BorderRadius.circular(20),border:Border.all(color:sel?orange:Colors.black12,width:sel?2:1),boxShadow:sel?[const BoxShadow(color:Colors.black12,blurRadius:8,offset:Offset(0,4))]:null),child:Row(children:[Container(width:28,height:28,decoration:BoxDecoration(shape:BoxShape.circle,border:Border.all(color:sel?orange:Colors.black38,width:2),color:sel?orange:Colors.transparent),child:sel?const Icon(Icons.check,color:Colors.white,size:18):null),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Text(p.$1,style:const TextStyle(fontSize:17,fontWeight:FontWeight.w900,color:ink)),if(i==1)Container(margin:const EdgeInsets.only(left:8),padding:const EdgeInsets.symmetric(horizontal:7,vertical:3),decoration:BoxDecoration(color:green,borderRadius:BorderRadius.circular(10)),child:const Text('PIÙ SCELTO',style:TextStyle(color:Colors.white,fontSize:9,fontWeight:FontWeight.w900)))]),Text(p.$3,style:const TextStyle(color:green,fontWeight:FontWeight.w700)),Text(p.$4,style:const TextStyle(fontSize:11,color:Colors.black54))])),Text(p.$2,style:const TextStyle(fontSize:22,fontWeight:FontWeight.w900,color:green))]))));}),
 const SizedBox(height:8),FilledButton(style:FilledButton.styleFrom(backgroundColor:orange,foregroundColor:ink,minimumSize:const Size.fromHeight(54),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(18))),onPressed:(){final p=plans[selectedPlan];ScaffoldMessenger.of(c).showSnackBar(SnackBar(content:Text('Piano ${p.$1}: il pagamento reale sarà collegato a Google Play Billing in fase di pubblicazione.')));},child:Text('Attiva ${plans[selectedPlan].$1} — ${plans[selectedPlan].$2}',style:const TextStyle(fontSize:16,fontWeight:FontWeight.w900))),const SizedBox(height:8),const Text('Puoi scegliere il piano prima del pagamento. Le condizioni di acquisto e rinnovo saranno mostrate chiaramente prima della conferma.',textAlign:TextAlign.center,style:TextStyle(fontSize:11,color:Colors.black54))]));}
}
...
class _RecipePageState extends State<RecipePage> with SingleTickerProviderStateMixin{
  ...
  Widget _recipeHero(Recipe r){
    return AnimatedBuilder(animation:heroController,builder:(context,child){
      ...
      DecoratedBox(decoration:BoxDecoration(gradient:LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[Colors.black.withOpacity(.05),Colors.transparent,Colors.black.withOpacity(.72)]))),
      Positioned(left:18,top:18,child:Container(padding:const EdgeInsets.symmetric(horizontal:12,vertical:7),decoration:BoxDecoration(color:Colors.white.withOpacity(.92),borderRadius:BorderRadius.circular(20)),child:Text('${flag(r.country)}  ${r.country}',style:const TextStyle(color:green,fontWeight:FontWeight.w900)))),
  ...
  Widget _ingredientsSection(Recipe r)=>Container(padding:const EdgeInsets.fromLTRB(15,16,15,14),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(28),border:Border.all(color:const Color(0xFFE7E1D6)),boxShadow:[BoxShadow(color:Colors.black.withOpacity(.045),blurRadius:16,offset:const Offset(0,6))]),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
  ...
  Widget _stepCard(int number,String text)=>TweenAnimationBuilder<double>(tween:Tween(begin:0,end:1),duration:Duration(milliseconds:350+number*45),curve:Curves.easeOutCubic,builder:(context,v,child)=>Opacity(opacity:v,child:Transform.translate(offset:Offset(0,18*(1-v)),child:child)),child:Container(margin:const EdgeInsets.only(bottom:12),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(23),boxShadow:[BoxShadow(color:Colors.black.withOpacity(.045),blurRadius:12,offset:const Offset(0,4))]),child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[Container(width:45,height:45,alignment:Alignment.center,decoration:const BoxDecoration(color:green,shape:BoxShape.circle),child:Text('$number',style:const TextStyle(color:Colors.white,fontSize:18,fontWeight:FontWeight.w900))),const SizedBox(width:13),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Passaggio $number',style:const TextStyle(color:green,fontWeight:FontWeight.w900,fontSize:12)),const SizedBox(height:4),Text(text,style:const TextStyle(fontSize:14,height:1.42,color:ink))])),const SizedBox(width:4),const Icon(Icons.chevron_right_rounded,color:Color(0xFFB7C9BF))])));

  Widget _historyAndVariants(Recipe r)=>Column(children:[Container(padding:const EdgeInsets.all(17),decoration:BoxDecoration(color:pale,borderRadius:BorderRadius.circular(25)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('La storia del piatto',style:TextStyle(fontSize:23,fontWeight:FontWeight.w900,color:ink)),const SizedBox(height:8),Text(r.history,style:const TextStyle(height:1.45))])),if(r.variants.isNotEmpty)Padding(padding:const EdgeInsets.only(top:14),child:Container(padding:const EdgeInsets.all(17),decoration:BoxDecoration(color:const Color(0xFFF6F0E4),borderRadius:BorderRadius.circular(25)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('Varianti della ricetta',style:TextStyle(fontSize:22,fontWeight:FontWeight.w900,color:ink)),const SizedBox(height:8),...r.variants.map((v)=>Padding(padding:const EdgeInsets.only(bottom:8),child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[const Icon(Icons.check_circle_outline,color:green,size:18),const SizedBox(width:8),Expanded(child:Text(v,style:const TextStyle(height:1.35)))]))])))]);

  Widget _bottomActions()=>SafeArea(top:false,child:Container(padding:const EdgeInsets.fromLTRB(12,9,12,8),decoration:BoxDecoration(color:Colors.white,borderRadius:const BorderRadius.vertical(top:Radius.circular(24)),boxShadow:[BoxShadow(color:Colors.black.withOpacity(.10),blurRadius:18,offset:const Offset(0,-5))]),child:Row(children:[Expanded(child:SizedBox(height:52,child:FilledButton.icon(onPressed:()=>jumpTo(preparationKey),icon:const Icon(Icons.restaurant_rounded),label:const Text('Inizia a cucinare',style:TextStyle(fontWeight:FontWeight.w900,fontSize:15))))),const SizedBox(width:8),SizedBox(width:56,height:52,child:FilledButton.tonal(onPressed:widget.onFavorite,child:Icon(isFavorite?Icons.favorite:Icons.favorite_border,color:isFavorite?Colors.red:green))),const SizedBox(width:8),SizedBox(width:56,height:52,child:FilledButton.tonal(onPressed:widget.onAddAll,child:const Icon(Icons.playlist_add,color:green)))])));
}
